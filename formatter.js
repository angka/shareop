...
function formatTime24Hour(timeString) {
  ...
  // If invalid format, return as-is (or empty)
  return timeString;
}
...
function formatJamWithStatus(jam, status_pasien) {
  ...
  const formattedTime = formatTime24Hour(jam);

  if (status_pasien && status_pasien.trim() !== '') {
    return `Jam ${formattedTime} (${status_pasien})`;
  } else {
    return `Jam ${formattedTime}`;
  }
}

function generateMessage(jadwalData, tanggal) {
